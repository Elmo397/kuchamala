...
fun generateHeader(title: String, textHtml: String, image: Image): String {
    val dropDownList =
...
fun generateContent(classesInfo: List<ClassPreview>): String {
    var content = ""
    var isLeft = true
...
fun createDurationCol(duration: Int) =
...
fun createImageContent(image: Image) =
    """[fildisi_single_image image_mode="large" image="${image.id}" image_full_column="yes"]"""

fun generateColumn(content: String) =
    """[vc_column width="1/2" tablet_sm_width="1" el_wrapper_class="column-height"]$content[/vc_column]"""

fun generateRow(bgColors: String, columns: String, elWrapper: String = "") =
    """[vc_row bg_type="color" equal_column_height="middle-content" mobile_equal_column_height="false" bg_color="$bgColors $elWrapper"]$columns[/vc_row]""".trimMargin()
